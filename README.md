...
디자인은 [`DESIGN.md`](DESIGN.md)의 Notion 톤을 따릅니다 — warm paper 캔버스(`#f6f5f4`)에 흰 카드, 구조 액센트는 파랑(`#0075de`) 하나, 아이콘은 Lucide. **그라데이션 CTA·진한 드롭섀도·이모지 남발 같은 "AI가 만든 티"는 규칙으로 금지**되어 있고 linter가 잡습니다. 색·라운드·버튼 치수는 `DESIGN.md` 한 곳에만 적혀 있어서, 다른 문서와 값이 갈릴 일이 없습니다.
...
그 컬럼에 안 잡히는 산출물 — 시안 허브·`common.css`·고객 문서 — 도 개별로 내용을 검사합니다. 셋 다 페이지 번호가 없어서 1:1 대조로는 영영 안 걸리는 자리입니다. `common.css`는 크기뿐 아니라 `:root` 토큰 블록이 살아 있는지와 주석 짝까지 봅니다 (주석 안에 글롭 `**/`가 들어가 뒤따르는 토큰이 통째로 무효화된 적이 있습니다 — 파일 크기는 멀쩡해서 크기 검사로는 안 잡혔습니다).
...
| 파일 | 역할 |
|---|---|
| [`CLAUDE.md`](CLAUDE.md) | 각 단계 산출물의 형식·절차 규칙 — 파이프라인의 **권위 문서** |
| [`DESIGN.md`](DESIGN.md) | 디자인 치수의 **유일한 출처** — 색·타이포·라운드·그림자 토큰, 버튼 5종, 모바일 프레임 |
| [`AGENT.md`](AGENT.md) | 오케스트레이터 실행 룰 + 마커 프로토콜 |
| `설명서.txt` | 사용법·점검 치트시트 (한국어) |
| `동작방식.txt` | 훅 판정 트리와 전체 흐름도 (한국어) |

디자인 톤을 바꾸려면 **`DESIGN.md`만 갈아끼우면 됩니다.** padding·radius·색 같은 치수는 전부 거기 한 곳에만 적혀 있고, `CLAUDE.md`와 에이전트 정의는 "무엇을 언제 쓰는가"만 다룹니다. 숫자가 어긋나면 항상 `DESIGN.md`가 이깁니다.
